...
def makePolygon(points):
    ...
    print("Number of vertices:  %i" % numVertices)
...
if __name__ == "__main__":
    ...
    with open("poly.txt","w") as txt_file:
        for i in range(len(pset)): # maybe -1 for len, because it loops around
            txt_file.write("%s %s \n" % (str(xs[i]), str(ys[i])))

    plt.text(xs[0], ys[0], "0")
    plt.text(xs[int(len(pset) / 4)], ys[int(len(pset) / 4)], "1")
    plt.text(xs[int(len(pset) / 2)], ys[int(len(pset) / 2)], "2")
